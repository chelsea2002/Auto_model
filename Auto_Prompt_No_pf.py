import pandas as pd
import time
from prompt import schema_linkings_prompt,debug_prompt,classification_prompt,easy_prompt,complex_prompt_no_pf,easy_prompt_gpt3_5
import json
import re
import os
...
OUTPUT_FILE = "log/spider/gpt4o_no_pf/predicted.sql"
...
def complex_prompt_maker(test_sample_text,table_match,database,schema_links):
    ...
    fields = find_fields_SL_like(database,table_match)
    schema_links = generate_mappings(schema_links)
    prompt = Instruct + complex_prompt_no_pf + f"### Input:{test_sample_text}"+f"\n### Database: {fields}" + f'\n###schema_links:{schema_links}'
    return prompt
